...
class VariationalAutoEncoder(object):
    ...
    def __read_img(self):
        ...
        return train_data
    # 训练模型(模型不存在，则构建模型)
    def train(self, epochs, batch_size=2):
        # 训练数据集进行归一化处理[0,1]
        train_data = self.__read_img() / 225
        vae = self.vae
        for i in range(epochs):
            vae.fit(train_data, batch_size=batch_size)
    ...
    def __vae(self):
        ...
        # 设置损失函数、优化器
        restructure_loss = binary_crossentropy(input, output)
        restructure_loss = K.mean(restructure_loss)
        print(restructure_loss.shape)
        # print('------------------')
        # print(restructure_loss.shape)
        # restructure_loss *= np.prod([320, 240, 3])
        ...
        # 如果模型存在
        if self.model_exist == True:
            vae.load_weights(self.model_path)
...
if __name__ == '__main__':
    vae = VariationalAutoEncoder(img_input=(320, 240, 3),
                                 folder_path=r'C:\Users\10651\Desktop\潘海辉\img',
                                 laten_dim=100,
                                 model_path='vae.h5',
                                 model_exist=True)
    vae.train(epochs=50, batch_size=20)
