...
def train_ae(model_path='', train_img_path='', epochs=500, batch_size=50):
    ...
    img_data = np.loadtxt('feature.txt')
    # 读取标签
...
def write_feature(train_img_path='C:\\Users\\yangqing\\Desktop\\新建文件夹\\train_img',
    ...
    # 读取img
    img_data = read_img(folder_path=train_img_path, img_input=(320, 256, 3))/225
    img_data = ae.predict(x=img_data, batch_size=4)
...
def predict_test_img(model_path, test_img_path):
    ...
    test_feature = ae.predict(test_img, batch_size=10)
    np.savetxt(str('test_feature.txt'),test_feature)
...
# 对图片进行特征提取，再保存到文件
# write_feature()
predict_test_img(model_path='convolutional_autoencoder.h5',
                 test_img_path='C:\\Users\\yangqing\\Desktop\\新建文件夹\\test_img')
# 进行可是化显示
